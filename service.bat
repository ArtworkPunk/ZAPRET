@echo off
set "LOCAL_VERSION= GITHUB VERSION FOR EVERYONE "
...
echo =========  !LOCAL_VERSION!  =========
echo 1. Install Bypass Service
echo 2. Remove Bypass Services
echo 3. Check Status
echo 4. Run Diagnostics
echo 5. Switch Game Filter (%GameFilterStatus%)
echo 6. Switch IP Filter (%IPsetStatus%)
echo 0. Exit
...
if !errorlevel!==0 (
    for /f "tokens=2*" %%A in ('reg query "HKLM\System\CurrentControlSet\Services\art_bypass" /v art_bypass-config 2^>nul') do echo Service config installed from "%%B"
)
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%winws.exe\" !ARGS!" DisplayName= "Artwork Bypass" start= auto
sc description %SRVCNAME% "Artwork DPI bypass for games and streaming"
sc start %SRVCNAME%
for %%F in ("!file%choice%!") do (
    set "filename=%%~nF"
)
reg add "HKLM\System\CurrentControlSet\Services\art_bypass" /v art_bypass-config /t REG_SZ /d "!filename!" /f
...
if not defined GITHUB_VERSION (
    echo Warning: failed to fetch the latest version. This warning does not affect the operation of the bypass
    timeout /T 9
    if "%1"=="soft" exit 
    goto menu
)
...
if !errorlevel!==0 (
    ...
    call :PrintRed "[X] Base Filtering Engine is not running. This service is required for bypass to work"
)
echo:
...
if !errorlevel!==0 (
    call :PrintRed "[X] Adguard process found. Adguard may cause problems with bypass"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] Killer services found. Killer conflicts with bypass"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] Intel Connectivity Network Service found. It conflicts with bypass"
) else (
...
if !checkpointFound!==1 (
    call :PrintRed "[X] Check Point services found. Check Point conflicts with bypass"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] SmartByte services found. SmartByte conflicts with bypass"
) else (
...
if !errorlevel!==0 (
    call :PrintYellow "[?] Some VPN services found. Some VPNs can conflict with bypass"
) else (
...
if !dohfound!==0 (
    call :PrintYellow "[?] Make sure you have configured secure DNS in a browser"
) else (
...
if not exist "%gameFlagFile%" (
    ...
    call :PrintYellow "Restart the bypass to apply the changes"
) else (
